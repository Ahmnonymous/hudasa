...
const foldersModel = {
  getAll: async (centerId = null) => {
    try {
      ...
      console.log(`[DEBUG] FoldersModel.getAll - SQL params: centerId=${centerId} (type: ${typeof centerId})`);
      
      const res = await pool.query(query, params);
      console.log(`[DEBUG] FoldersModel.getAll - SQL success: ${res.rows.length} rows returned`);
      return res.rows;
    } catch (err) {
      console.error(`[ERROR] FoldersModel.getAll - SQL error: ${err.message}`, err);
      throw new Error("Error fetching all records from Folders: " + err.message);
    }
  },
  ...
  getById: async (id, centerId = null) => {
    try {
      ...
      throw new Error("Error fetching record by ID from Folders: " + err.message);
    }
  },
  ...
  create: async (fields) => {
    try {
      ...
      throw new Error("Error creating record in Folders: " + err.message);
    }
  },
  ...
  update: async (id, fields, centerId = null) => {
    try {
      ...
      throw new Error("Error updating record in Folders: " + err.message);
    }
  },
  ...
  delete: async (id, centerId = null) => {
    try {
      ...
      throw new Error("Error deleting record from Folders: " + err.message);
    }
  }
};
