...
const tableName = 'Site_Visits';

const siteVisitsModel = {
  getAll: async (centerId = null, isMultiCenter = false) => {
    try {
      ...
      throw new Error("Error fetching all records from Site_Visits: " + err.message);
    }
  },
  ...
  getById: async (id, centerId = null, isMultiCenter = false) => {
    try {
      ...
      throw new Error("Error fetching record by ID from Site_Visits: " + err.message);
    }
  },

  getByIslamicCenterId: async (islamicCenterId, centerId = null, isMultiCenter = false) => {
    try {
      let where = `islamic_center_id = $1`;
      const params = [islamicCenterId];
      ...
      throw new Error("Error fetching records by Islamic Center ID from Site_Visits: " + err.message);
    }
  },
  ...
  create: async (fields) => {
    try {
      ...
      throw new Error("Error creating record in Site_Visits: " + err.message);
    }
  },
  ...
  update: async (id, fields, centerId = null, isMultiCenter = false) => {
    try {
      ...
      throw new Error("Error updating record in Site_Visits: " + err.message);
    }
  },
  ...
  delete: async (id, centerId = null, isMultiCenter = false) => {
    try {
      ...
      throw new Error("Error deleting record from Site_Visits: " + err.message);
    }
  }
};

module.exports = siteVisitsModel;
