...
const AcademicResultsTab = ({ application, academicResults, onUpdate, showAlert, lookupData = {} }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (editItem) {
        await axiosApi.put(
          `${API_BASE_URL}/academicResults/${editItem.id}`,
          formData,
          {
            headers: { "Content-Type": "multipart/form-data" },
          }
        );
        showAlert("Academic results updated successfully", "success");
      } else {
        await axiosApi.post(`${API_BASE_URL}/academicResults`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        showAlert("Academic results created successfully", "success");
      }
      ...
      showAlert(
        error.response?.data?.error || "Failed to save academic results",
        "danger"
      );
    }
  };
  ...
  const handleDelete = () => {
    ...
    const resultName = editItem.school || editItem.term || editItem.grade || 'Academic Result';
    
    showDeleteConfirmation({
      id: editItem.id,
      name: resultName,
      type: "academic result",
      message: "This academic result will be permanently removed from the system."
    }, async () => {
      try {
        await axiosApi.delete(`${API_BASE_URL}/academicResults/${editItem.id}`);
        showAlert("Academic result has been deleted successfully", "success");
        onUpdate();
        if (modalOpen) {
          setModalOpen(false);
        }
        ...
        showAlert(error?.response?.data?.message || "Failed to delete academic result", "danger");
  ...
  return (
    <>
      <div className="mb-3 d-flex justify-content-between align-items-center">
        <h5 className="mb-0">Academic Results</h5>
        {!isOrgExecutive && (
      ...
      {academicResults && academicResults.length > 0 ? (
        ...
        <div className="alert alert-info" role="alert">
          ...
          No academic results found. Click "Add Results" to create one.
        </div>
      )}
      ...
      <Modal isOpen={modalOpen} toggle={toggleModal} centered size="xl" backdrop="static">
        <ModalHeader toggle={toggleModal}>
          ...
          {editItem ? "Edit" : "Add"} Academic Results
        </ModalHeader>
        <Form onSubmit={handleSubmit(onSubmit)}>
          <ModalBody style={{ maxHeight: "70vh", overflowY: "auto" }}>
            <Row>
              ...
              <Col md={12}>
                <hr />
                <h6 className="mb-3">Subject Results</h6>
              </Col>
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          ...
                          >
                            ...
                            {lookupData.academicSubjects?.map((subject) => (
...
export default AcademicResultsTab;
